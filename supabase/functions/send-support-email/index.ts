...
const SUPPORT_EMAIL = "nadine.katschmarek@googlemail.com";

const SUBJECT_LABELS: Record<string, string> = {
  account_setup: "Neues Konto einrichten",
  login_issues: "Login-Probleme",
...
async function incrementRateLimit(
  ...
  if (!existing) {
    await supabase.from("support_rate_limits").insert({
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background: #003366; color: white; padding: 20px; border-radius: 8px 8px 0 0;">
    <h1 style="margin: 0; font-size: 24px;">Support-Anfrage</h1>
    <p style="margin: 10px 0 0 0; opacity: 0.9;">Quartier Spots</p>
  </div>
  ...
  <div style="padding: 15px; text-align: center; color: #666; font-size: 12px;">
    <p style="margin: 0;">Gesendet via Quartier Spots Support-Formular</p>
  </div>
</body>
</html>`.trim();
}
...
serve(async (req) => {
  ...
  try {
    ...
    const [emailLimit, ipLimit] = await Promise.all([
      checkRateLimit(supabase, email, "email"),
      checkRateLimit(supabase, clientIP, "ip"),
    ]);
    ...
    const subjectLabel = SUBJECT_LABELS[subject] || subject;
    const htmlContent = createEmailHtml(name.trim(), email.trim(), subject, message.trim());
    ...
    const resendResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: `Quartier Spots <onboarding@resend.dev>`,
        to: [SUPPORT_EMAIL],
    ...
    await Promise.all([
      incrementRateLimit(supabase, email, "email"),
      incrementRateLimit(supabase, clientIP, "ip"),
    ]);
